...
@router.post('/')
async def set_payment(new_payment: PaymentBase, user: UserId = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.delete('/')
async def delete_payment(user: UserId = Depends(get_current_user), db: Session = Depends(get_db)):
    result = set_user_payment(
        db=db, email=user.email, payment=Payment(next_billing=None)
    )
    return {'success': True if result else False}
